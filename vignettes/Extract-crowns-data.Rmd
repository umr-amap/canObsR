...
labeling_file <- openxlsx::read.xlsx(file.path(path,paste(site,'labelingFile',paste0(format(as.Date(Sys.Date(),format="%Y-%m-%d"), format = "%Y%m%d"), '.xlsx'), sep = '_' )))

labeling_file$date <- as.Date(labeling_file$date, "%Y_%m_%d")
shiny_labels(data_labeling = labeling_file) # Tester app (text pheno et plot reste à faire)
```
