...
#'
#' @param path_in character. Path to the target images, or to a folder containing multiple target images. Images must be of Geotiff format.
#' @param ref_filepath character. Path to the reference image.
#' @param out_dir_path character. Directory where the outputs are saved.
#' @param corr_type character. Type of co-registration. Either 'global' (default) or 'local'.
#' @param max_shift integer. Maximum shift distance in reference image pixel units
...
#' @param grid_res integer. Tie point grid resolution in pixels of the target image (x-direction). Only applies to local co-registration.
