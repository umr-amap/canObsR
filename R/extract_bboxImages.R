...
extract_bboxImages <-
   ...
   function(RGB_paths, crs, date, directory = NULL, filename = NULL){
      ...
      for (i in 1:length(RGB_paths)) {
         ...
         if(!is.null(directory)){sf::write_sf(sf, paste0(directory, filename ,'_', date[i], '.gpkg') )}

         assign(paste0('bbox_',date[i]), bbox)
      }

      my_list <- mget(ls(patter = "bbox_"))
      sf <- do.call("rbind", lapply(my_list, sf::st_sf))
